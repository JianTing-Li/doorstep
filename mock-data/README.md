...
Run `python3 mock-data/validate.py` after touching anything here. It checks all 95 invariants referenced below and exits non-zero on a break.

---

## Rules

1. **`service_type` is always an array**, even with a single value — `["plumbing"]`, never `"plumbing"`.
2. **Compare on `code`, display `label`.** Every service type has a machine `code` stored in the data and a human `label` shown on screen. Filter and match on `code`; look up `label` only when rendering. Never compare against a label.
3. **Products B and C show only `listing_status: "active"`.** `draft`, `paused`, `suspended`, and `archived` listings exist for Products A and D — they must never reach customer browse, search, or chatbot matches.
4. **`_meta.reference_date` is "today".** Don't use the system clock for anything date-relative, or availability and booking history will look wrong.
5. **`booking_id` is not unique in `reviews.json`.** A booking can carry more than one review. Group by `booking_id` and take the latest `created_at` if you need one review per booking.
6. **Nobody edits these files, or adds a service type, without telling the team first.** Four products read the same IDs — a renamed field or a ninth code silently breaks somebody else's build. Run `validate.py` before committing a change.
7. **Treat every import as read-only.** Copy the data if you need to mutate it; keep any product-specific state (drafts, filters, session data) in your own store, not in these files.
...
Product C matches customer text against `listing_description` and `service_type`, writes bookings with `source: "chatbot"`, and keeps conversation state in its own session — not in these files.
...
| `service_type` | array of string | One or more codes (rule 1) |
...
| `booking_id` | string | FK → `bookings.json`, always a `completed` booking. **Not unique** — see rule 5 |
